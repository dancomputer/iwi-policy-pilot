...
Inputs:
  - predicted_YYYY.csv: columns [pixel, prediction] (or [Index_ID, Prediction])
...
Outputs:
  1) payouts_per_pixel_YYYY.csv  with columns: ['Pixel ID', 'Modelled Yield', 'Payout%', 'Payout']
  2) payouts_per_farmer_YYYY.csv with columns: ['Pixel ID', 'Farmer SN', 'Payout']
...
def _extract_year_from_filename(filepath: str) -> Optional[str]:
    """Extract 4-digit year from filename like 'predicted_2022.csv' or 'predicted_2023.csv'"""
...
def make_payout_dfs(predicted_csv_path: str,
    ...
    year = _extract_year_from_filename(predicted_csv_path)
    if not year:
        print("Warning: Could not extract year from filename, defaulting to 'YYYY'")
        year = "YYYY"
    ...
    if save_csv:
        ...
        df_pixel.to_csv(out_dir / f"payouts_per_pixel_{year}.csv", index=False)
        df_farmer.to_csv(out_dir / f"payouts_per_farmer_{year}.csv", index=False)
        print(f"Saved: {out_dir / f'payouts_per_pixel_{year}.csv'}")
        print(f"Saved: {out_dir / f'payouts_per_farmer_{year}.csv'}")

    return df_pixel, df_farmer


def _main():
    ap = argparse.ArgumentParser(description="Compute payouts and apportion to farmers using NKASI workbook + df_final meta.")
    ap.add_argument("--predicted", required=True, help="Path to predicted_YYYY.csv (columns: pixel, prediction)")
...
if __name__ == "__main__":
  ...
  #--metadata_csv "C:\Users\danie\NecessaryM1InternshipCode\ProjectRice\OutputCalendarDays180_Maize_1982_2021_SPARSE\ThreeVariableContiguous-SyntheticYield-Optimistic-metadata.csv"
    df_pixel, df_farmer = make_payout_dfs(
        predicted_csv_path=r"C:\Users\danie\NecessaryM1InternshipCode\ProjectRice\OutputCalendarDays180_PredictionMaize_1981_2023_SPARSE\predicted_2023.csv",
        ...
        metadata_csv=r"C:\Users\danie\NecessaryM1InternshipCode\ProjectRice\OutputCalendarDays180_Maize_1981_2022_SPARSE\ThreeVariableContiguous-SyntheticYield-Optimistic-metadata.csv")
